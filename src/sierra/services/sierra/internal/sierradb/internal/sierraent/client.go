...
import (
	...
	"sierra/services/sierra/internal/sierradb/internal/sierraent/migrate"
...
func (c *Client) Use(hooks ...Hook) {
	c.SampleFile.Use(hooks...)
}

// Intercept adds the query interceptors to all the entity clients.
// In order to add interceptors to a specific client, call: `client.Node.Intercept(...)`.
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.SampleFile.Intercept(interceptors...)
}
...
func (c *SampleFileClient) MapCreateBulk(slice any, setFunc func(*SampleFileCreate, int)) *SampleFileCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &SampleFileCreateBulk{err: fmt.Errorf("calling to SampleFileClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*SampleFileCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
func (c *SampleFileClient) UpdateOneID(id int) *SampleFileUpdateOne {
...
func (c *SampleFileClient) DeleteOneID(id int) *SampleFileDeleteOne {
...
func (c *SampleFileClient) Get(ctx context.Context, id int) (*SampleFile, error) {
	return c.Query().Where(samplefile.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *SampleFileClient) GetX(ctx context.Context, id int) *SampleFile {
...
func (c *SampleFileClient) mutate(ctx context.Context, m *SampleFileMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("sierraent: unknown SampleFile mutation op: %q", m.Op())
	}
}
