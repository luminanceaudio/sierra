...
CREATE TABLE `new_source_samples` (`uri` text NOT NULL, `relative_path` text NOT NULL, `filename` text NOT NULL, `source` text NULL, `sample` text NULL, PRIMARY KEY (`uri`), CONSTRAINT `source_samples_sources_source` FOREIGN KEY (`source`) REFERENCES `sources` (`uri`) ON DELETE SET NULL, CONSTRAINT `source_samples_samples_sample` FOREIGN KEY (`sample`) REFERENCES `samples` (`sha256`) ON DELETE SET NULL);
-- copy rows from old table "source_samples" to new temporary table "new_source_samples"
INSERT INTO `new_source_samples` (`uri`, `relative_path`, `source`, `sample`) SELECT `uri`, `relative_path`, `source`, `sample` FROM `source_samples`;
