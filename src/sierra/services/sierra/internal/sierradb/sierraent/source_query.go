...
func (sq *SourceQuery) QuerySample() *SourceSampleQuery {
...
func (sq *SourceQuery) Clone() *SourceQuery {
	...
	return &SourceQuery{
		...
		withSample: sq.withSample.Clone(),
...
func (sq *SourceQuery) WithSample(opts ...func(*SourceSampleQuery)) *SourceQuery {
	query := (&SourceSampleClient{config: sq.config}).Query()
...
//
//	client.Source.Query().
//		GroupBy(source.FieldCreateTime).
...
func (sq *SourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Source, error) {
	...
	if query := sq.withSample; query != nil {
		if err := sq.loadSample(ctx, query, nodes,
			func(n *Source) { n.Edges.Sample = []*SourceSample{} },
			func(n *Source, e *SourceSample) { n.Edges.Sample = append(n.Edges.Sample, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SourceQuery) loadSample(ctx context.Context, query *SourceSampleQuery, nodes []*Source, init func(*Source), assign func(*Source, *SourceSample)) error {
	...
	query.Where(predicate.SourceSample(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(source.SampleColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.source
		if fk == nil {
			return fmt.Errorf(`foreign-key "source" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "source" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
