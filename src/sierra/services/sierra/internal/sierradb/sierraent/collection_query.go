...
func (cq *CollectionQuery) QuerySample() *SourceSampleQuery {
	query := (&SourceSampleClient{config: cq.config}).Query()
...
func (cq *CollectionQuery) QueryCollectionSamples() *CollectionSampleQuery {
	query := (&CollectionSampleClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CollectionQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (cq *CollectionQuery) Clone() *CollectionQuery {
	...
	return &CollectionQuery{
		...
		withSample:            cq.withSample.Clone(),
		withCollectionSamples: cq.withCollectionSamples.Clone(),
...
func (cq *CollectionQuery) WithSample(opts ...func(*SourceSampleQuery)) *CollectionQuery {
	query := (&SourceSampleClient{config: cq.config}).Query()
...
func (cq *CollectionQuery) WithCollectionSamples(opts ...func(*CollectionSampleQuery)) *CollectionQuery {
	query := (&CollectionSampleClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCollectionSamples = query
	return cq
}
...
func (cq *CollectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Collection, error) {
	...
	if query := cq.withSample; query != nil {
		if err := cq.loadSample(ctx, query, nodes,
			func(n *Collection) { n.Edges.Sample = []*SourceSample{} },
			func(n *Collection, e *SourceSample) { n.Edges.Sample = append(n.Edges.Sample, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCollectionSamples; query != nil {
		if err := cq.loadCollectionSamples(ctx, query, nodes,
			func(n *Collection) { n.Edges.CollectionSamples = []*CollectionSample{} },
			func(n *Collection, e *CollectionSample) {
...
func (cq *CollectionQuery) loadSample(ctx context.Context, query *SourceSampleQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *SourceSample)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Collection)
	nids := make(map[string]map[*Collection]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(collection.SampleTable)
		s.Join(joinT).On(s.C(sourcesample.FieldID), joinT.C(collection.SamplePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(collection.SamplePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(collection.SamplePrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*SourceSample](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "sample" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (cq *CollectionQuery) loadCollectionSamples(ctx context.Context, query *CollectionSampleQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *CollectionSample)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(collectionsample.FieldCollectionID)
	}
	query.Where(predicate.CollectionSample(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(collection.CollectionSamplesColumn), fks...))
	}))
